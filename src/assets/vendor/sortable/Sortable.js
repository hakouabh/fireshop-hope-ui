...
(function (global, factory) {
  ...
  function dispatchEvent (_ref) {
    ...
    var onName = 'on' + name.charAt(0).toUpperCase() + name.substr(1) // Support for new CustomEvent feature
  ...
  var pluginEvent = function pluginEvent (eventName, sortable) {
    ...
    var data = _objectWithoutProperties(_ref, ['evt'])
  ...
  /** @lends Sortable.prototype */
  {
    ...
    /** TouchEvent */
      ...
      if (tapEvt) {
        ...
        var dx = (touch.clientX - tapEvt.clientX + fallbackOffset.x) / (scaleX || 1) + (relativeScrollOffset ? relativeScrollOffset[0] - ghostRelativeParentInitialScroll[0] : 0) / (scaleX || 1)
        var dy = (touch.clientY - tapEvt.clientY + fallbackOffset.y) / (scaleY || 1) + (relativeScrollOffset ? relativeScrollOffset[1] - ghostRelativeParentInitialScroll[1] : 0) / (scaleY || 1) // only set the status to dragging, when we are actually dragging
    ...
    _appendGhost: function _appendGhost () {
      ...
      if (!ghostEl) {
        ...
        var options = this.options // Position absolutely
    ...
    /** Event */
      evt) {
      var el = this.el
      var options = this.options // Get the index of the dragged element within its parent
  ...
  function _onMove (fromEl, toEl, dragEl, dragRect, targetEl, targetRect, originalEvent, willInsertAfter) {
    ...
    var retVal // Support for new CustomEvent feature
  ...
  function AutoScrollPlugin () {
    ...
    AutoScroll.prototype = {
      ...
      _handleAutoScroll: function _handleAutoScroll (evt, fallback) {
        ...
        var y = (evt.touches ? evt.touches[0] : evt).clientY
  ...
  var autoScroll = throttle(function (evt, options, rootEl, isFallback) {
    ...
    var scrollCustomFn // New scroll root, set scrollEl
  ...
  function MultiDragPlugin () {
    ...
    MultiDrag.prototype = {
      ...
      drop: function drop (_ref12) {
        ...
        if (!evt) return
        var options = this.options
        var children = parentEl.children // Multi-drag selection
